...
This repository contains documentation only. Security issues in this repo are limited to:

### In Scope
- Documentation that incorrectly describes security-sensitive flows (key handling, consent signing, recovery)
- Documentation that could mislead implementors into insecure patterns
- Exposed credentials or secrets accidentally committed to docs

### Out of Scope
- The documentation content itself does not execute code — implementation vulnerabilities should be reported against the relevant repository (bsp-sdk-typescript, bsp-mcp, bsp-id-web, bsp-registry-api)
