...
Five Move modules enforce the protocol on Aptos. Once deployed, they are permanent.
...
4. **Zero-Trust Check:** The Relayer API fetches the user's public key from the Arweave `BEORegistry`. It mathematically verifies the signature against the payload.
   ...
   - ✅ **Valid Signature:** The API wraps the user's exact intent in an Aptos transaction, pays the gas using the Ambrósio Institute's wallet, and submits it to the BSP smart contracts on Aptos.
