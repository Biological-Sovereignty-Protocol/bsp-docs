...
Smart contracts (Move modules) managing **BEO** identities, .bsp domain registrations, and access permissions run on Aptos — ensuring that the rules of the protocol cannot be changed by any single actor. All critical parameter changes require multi-signature authorization from Institute keyholders.
